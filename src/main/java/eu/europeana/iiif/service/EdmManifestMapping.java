...
public final class EdmManifestMapping {
   ...
   static String getNavDate(String europeanaId, Object jsonDoc) {
    ...
    /**
     * @return the {@link eu.europeana.iiif.model.v3.Canvas} that refers to edmIsShownBy, or else just the first SummaryCanvas
     */
     ...
     *  SummaryCanvas. Null if there are no canvases
     */
    ...
    private static String lookupServiceDoapImplements(Map<String, Object>[] services, String serviceId, String europeanaId) {
        ...
        if (result == null) {
            LOG.warn("SummaryManifest {} defined service {} in webresource, but no such service is defined (with a doapImplements field)", europeanaId, serviceId);
        }
        return result;
    }
    ...
    private static String getDoapImplementsValue(Object doapImplements, String europeanaId, String serviceId) {
        if (doapImplements == null) {
            LOG.warn("SummaryManifest {} has service {} with no doapImplements field", europeanaId, serviceId);
        } else if (doapImplements instanceof List) {
            List<String> diList = (List<String>) doapImplements;
            if (diList.isEmpty()) {
                LOG.warn("SummaryManifest {} has service {} with empty doapImplements list", europeanaId, serviceId);
            } else {
                return diList.get(0);
            }
        } else {
